...
/* cache data block accessor */
#define __CACHE_ACCESS(type, data, bofs)				\
  (*((type *)(((char *)data) + (bofs))))

/* cache data block accessors */
...
/* copy data out of a cache block */
#define CACHE_BCOPY(cmd, blk, bofs, p, nbytes)	\
  if (cmd == Read)							\
    {									\
      switch (nbytes) {							\
      case 1: *((byte_t *)p) = CACHE_BYTE(&blk->data[0], bofs); break;	\
      case 2: *((half_t *)p) = CACHE_HALF(&blk->data[0], bofs); break;	\
      case 4: *((word_t *)p) = CACHE_WORD(&blk->data[0], bofs); break;	\
      default:								\
	{ int words = nbytes >> 2;					\
	  while (words-- > 0) { *((word_t *)p) = CACHE_WORD(&blk->data[0], bofs); \
	      p += 4; bofs += 4; } } } }				\
  else {								\
      switch (nbytes) {							\
      case 1: CACHE_BYTE(&blk->data[0], bofs) = *((byte_t *)p); break;	\
      case 2: CACHE_HALF(&blk->data[0], bofs) = *((half_t *)p); break;	\
      case 4: CACHE_WORD(&blk->data[0], bofs) = *((word_t *)p); break;	\
      default:								\
	{ int words = nbytes >> 2;					\
	  while (words-- > 0) { CACHE_WORD(&blk->data[0], bofs) = *((word_t *)p); \
	      p += 4; bofs += 4; } } } }

/* bound function */
#define BOUND_POS(N)		((int)(MIN(MAX(0, (N)), 2147483647)))

/* unlink BLK from hash bucket chain */
static void unlink_htab_ent(struct cache_t *cp, struct cache_set_t *set, struct cache_blk_t *blk)
{
...
/* link BLK to hash table */
static void link_htab_ent(struct cache_t *cp, struct cache_set_t *set, struct cache_blk_t *blk)
{
  int index = CACHE_HASH(cp, blk->tag);
...
/* update way list */
static void update_way_list(struct cache_set_t *set, struct cache_blk_t *blk, enum list_loc_t where)
{
...
cache_create(char *name, int nsets, int bsize, int balloc, int usize,
             ...
             unsigned int (*blk_access_fn)(enum mem_cmd cmd, md_addr_t baddr,
                                           int bsize, struct cache_blk_t *blk,
                                           tick_t now),
...
void cache_reg_stats(struct cache_t *cp, struct stat_sdb_t *sdb)
{
  ...
  name = (!cp->name || !cp->name[0]) ? "<unknown>" : cp->name;
  ...
  /* NEW PREFETCH STAT */
  sprintf(buf, "%s.prefetches_issued", name);
  stat_reg_counter(sdb, buf, "Total number of prefetches issued", &cp->prefetch_count, 0, NULL);

  sprintf(buf, "%s.miss_rate", name);
  sprintf(buf1, "%s.misses / %s.accesses", name, name);
  stat_reg_formula(sdb, buf, "miss rate", buf1, NULL);
}
...
void cache_stats(struct cache_t *cp, FILE *stream)
{
  ...
  fprintf(stream,
	  ...
	  cp->name,
	  (double)cp->misses/sum, (double)cp->replacements/sum,
	  (double)cp->invalidations/sum);
}

/* access a cache */
unsigned int cache_access(struct cache_t *cp, enum mem_cmd cmd,
	     ...
	     byte_t **udata, md_addr_t *repl_addr)
{
  ...
  if (!cp->in_prefetch)
  {
    ...
    {
      cp->in_prefetch = 1;
      cache_access(cp, Read, next_line, NULL, cp->bsize, now, NULL, NULL);
      cp->prefetch_count++;
      cp->in_prefetch = 0;
    }
    else if (mode && strcmp(mode, "oneblock") == 0)
    {
      ...
      cp->in_prefetch = 1;
      cache_access(cp, Read, next_block, NULL, cp->bsize, now, NULL, NULL);
      cp->prefetch_count++;
      cp->in_prefetch = 0;
    }
    else if (mode && strcmp(mode, "stride") == 0)
    {
      ...
      {
        cp->in_prefetch = 1;
        cache_access(cp, Read, addr + stride, NULL, cp->bsize, now, NULL, NULL);
        cp->prefetch_count++;
        cp->in_prefetch = 0;
      }
      stride = new_stride;
