/* Patched memlat.c - safe pointer chaining and bounds checks
 *
 * Replaces compute_forward_pointers() and compute_backward_pointers()
 * with index-based implementations and adds allocation/rounding safety.
 *
 * Compile: gcc -O2 -pthread memlat.c -o memlat
 */
...
static struct option long_options[] = {
    {"line-size", required_argument, 0, 'l'},
...
/* Utility: build safe pointer chains with line indices */
/* mem: buffer, size: bytes (must be rounded to multiple of opt_line_sz) */
static void compute_forward_pointers(char *mem, unsigned long size)
{
    unsigned long nlines = size / opt_line_sz;
    if (nlines == 0)
        return;

    /* stride in lines (must be integer because opt_stride % opt_line_sz == 0) */
    unsigned long stride_lines = opt_stride / opt_line_sz;

    for (unsigned long line = 0; line < nlines; ++line) {
        unsigned long target_line = (line + stride_lines) % nlines;
        void **slot = (void **)(mem + line * opt_line_sz);
        *slot = (void *)(mem + target_line * opt_line_sz);
    }
}
...
static void compute_backward_pointers(char *mem, unsigned long size)
{
    ...
    for (unsigned long line = 0; line < nlines; ++line) {
        unsigned long target_line = (line + stride_lines) % nlines;
        /* write pointer at target_line pointing back to line */
        void **slot = (void **)(mem + target_line * opt_line_sz);
        *slot = (void *)(mem + line * opt_line_sz);
    }
}
...
static void *run_test(void *arg)
{
    ...
    /* Round requested max_size up to a multiple of line size to simplify indexing */
    unsigned long alloc_size = ((opt_max_size + opt_line_sz - 1) / opt_line_sz) * opt_line_sz;
    if (alloc_size == 0) {
        fprintf(stderr, "Requested allocation size is zero\n");
        exit(1);
    }

    char *mem = malloc(alloc_size);
    if (!mem) {
        fprintf(stderr, "Error allocating memory (%lu bytes): %s\n",
            alloc_size, strerror(errno));
        exit(1);
    }

    /* initialize memory a bit to avoid undefined reads in some environments */
    memset(mem, 0x5A, alloc_size);

    warmup_memory(mem, alloc_size);

    /* iterate sizes; ensure each 'size' is a multiple of opt_line_sz */
    unsigned long size;
    for (size = (MIN_SIZE < opt_stride ? opt_stride : MIN_SIZE);
         size <= opt_max_size;
         /* step function from original - keep same stepping but round to line */
         size = step(size)) {

        /* Round this test size down to a multiple of opt_line_sz and not exceed alloc_size */
        unsigned long test_size = (size / opt_line_sz) * opt_line_sz;
        if (test_size == 0)
            test_size = opt_line_sz;
        if (test_size > alloc_size)
            test_size = alloc_size;

        double latency = scan(id, mem, test_size);

        printf("%ld, %.5f, %.3f\n", id, (double)test_size / (1024 * 1024),
               latency);
    }
